...
vi.stubGlobal('createImageBitmap', vi.fn(async (imageData: { width: number; height: number }) => ({
...
vi.mock('@tauri-apps/api/core', () => ({
  Channel: class implements MockChannel {
    ...
    set onmessage(fn: ChannelHandler) { this._handler = fn; }
    triggerMessage(msg: BackendRenderArtifact) { this._handler?.(msg); }
...
function makeRawArtifact(overrides: Partial<BackendRenderArtifact> = {}): BackendRenderArtifact {
  return {
    ...
    rgba_data: Array.from({ length: 80 * 45 * 4 }, () => 128),
    width: 80,
    height: 45,
    timestamp_ms: 1000,
    source: 'fresh-decode',
...
describe('requestProgressiveTiers', () => {
  ...
  it('delivers L0 then L1 in sequence', async () => {
    ...
    channelInstances[1]?.triggerMessage(makeRawArtifact({ spatial_tier: SpatialTier.L1, width: 120, height: 67 }));
    await new Promise(r => setTimeout(r, 20));
