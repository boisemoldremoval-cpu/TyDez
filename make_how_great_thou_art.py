#!/usr/bin/env python3
"""
make_how_great_thou_art.py — a full instrumental worship rendition of the hymn
"How Great Thou Art".

The melody is the PUBLIC-DOMAIN Swedish folk tune "O Store Gud" (Carl Boberg,
1885), transcribed to D major. The modern English lyrics (Stuart K. Hine, 1949)
are under copyright and are NOT included — this renders the tune only.

Form: reverent solo-guitar intro -> Verse 1 (guitar + strings) -> Refrain 1
(brushed drums & bass swell in) -> Verse 2 (fuller) -> Refrain 2 (full, peak)
-> reverent tag. Same instrument set throughout; transitions are gradual gain
automations. Pure-numpy synthesis; no external service.

Usage:
    pip install numpy imageio-ffmpeg
    python3 make_how_great_thou_art.py            # -> how_great_thou_art.mp3
    python3 make_how_great_thou_art.py out.mp3
"""
...
# --- "How Great Thou Art" melody in D major (public-domain tune) ------------
# do=62 re=64 mi=66 fa=67 sol=69 la=71 ti=73 do'=74 re'=76 mi'=78
...
VERSE_MEL = LINE_A + LINE_B + LINE_A + LINE_B2            # 64 beats (16 bars)

REF_A = [(69,1),(69,1),(74,2),(78,2),(76,1),(74,1),(73,1),(74,1),(71,1),(69,3),(0,2)]
REF_B = [(74,1),(74,1),(73,2),(76,2),(78,2),(76,1),(74,1),(74,4),(0,2)]
REFRAIN_MEL = REF_A + REF_B + REF_A + REF_B               # 64 beats (16 bars)

INTRO_MEL = [(62,2),(64,2),(66,4),(69,4),(0,4)]           # 16 beats (4 bars)
OUTRO_MEL = REF_A + REF_B                                 # 32 beats (8 bars)

# --- chords per bar --------------------------------------------------------
INTRO_CH   = ['D','G','A','D']
VERSE_CH   = ['D','D','G','A','D','G','D','D','D','D','G','A','G','D','A','D']
REFRAIN_CH = ['D','G','D','A','D','A','D','D','G','D','G','A','D','A','D','D']
OUTRO_CH   = ['D','A','Bm','G','D','A','G','D']

# name, bars, level@start, sparse-pick, melody, chords
SECTIONS = [
    ('Intro',     4,  dict(drm=0.0, bass=0.0, strg=0.42, gtr=0.70, lead=0.4), True,  INTRO_MEL,   INTRO_CH),
    ('Verse 1',   16, dict(drm=0.0, bass=0.5, strg=0.55, gtr=0.90, lead=0.72),True,  VERSE_MEL,   VERSE_CH),
    ('Refrain 1', 16, dict(drm=0.5, bass=0.8, strg=0.82, gtr=0.95, lead=0.86),False, REFRAIN_MEL, REFRAIN_CH),
    ('Verse 2',   16, dict(drm=0.58,bass=0.82,strg=0.78, gtr=0.95, lead=0.82),False, VERSE_MEL,   VERSE_CH),
    ('Refrain 2', 16, dict(drm=1.0, bass=0.95,strg=1.00, gtr=1.00, lead=0.96),False, REFRAIN_MEL, REFRAIN_CH),
    ('Outro',     8,  dict(drm=0.0, bass=0.0, strg=0.55, gtr=0.70, lead=0.6), True,  OUTRO_MEL,   OUTRO_CH),
]
...
mix = mix + 0.34 * reverb(mix)                             # a little wetter for the hymn
...
out_mp3 = sys.argv[1] if len(sys.argv) > 1 else 'how_great_thou_art.mp3'
